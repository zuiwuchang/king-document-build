...
c)g.Show(b["warning.title"],b["data not need save"]);else{var e=this;$.ajax({url:"/Section/AjaxSave",type:"POST",dataType:"json",data:{id:a,val:c}}).done(function(a){0==a.Code?(d=c,e.UpdateStatus()):g.Show(b["err.title"],a.Emsg)}).fail(function(){g.Show(b["err.title"],b["err net"])})}},UpdateStatus:function(a){a?a==d?q.text(b["section ok"]):q.text(b["section no"]):q.text(b["section ok"])}},m=v.find(".kEditor");(d||""==d)&&m.val(d);var u=KindEditor.create(m,{uploadJson:"/Files/Upload?id="+a,fileManagerJson:"/Files/Find?id="+
a,allowFileManager:!0,urlType:"relative",resizeType:1,width:"100%",height:400,items:["source","|","undo","redo","|","mycode","|","cut","copy","paste","plainpaste","|","justifyleft","justifycenter","justifyright","justifyfull","insertorderedlist","insertunorderedlist","indent","outdent","subscript","superscript","clearhtml","selectall","|","fullscreen","/","bold","italic","underline","strikethrough","lineheight","removeformat","|","image","insertfile","table","link","unlink",,],afterCreate:function(){n._isCreate=
!0},afterChange:function(){if(n._isCreate){var b=this.html();n.UpdateStatus(b)}}});u._k_ctx=n;d||""==d?n.ShowPreview(d):d="";var w=function(d){c!=d&&1==e&&(e=2,$.ajax({url:"/Section/AjaxRename",type:"POST",dataType:"json",data:{id:a,name:d}}).done(function(a){0==a.Code?(c=d,E.text(c),x.Name=c):g.Show(b["err.title"],a.Emsg)}).fail(function(){g.Show(b["err.title"],b["err net"])}).always(function(){e=1}))},y=function(c){1==e&&(e=2,$.ajax({url:"/Section/AjaxRemove",type:"POST",dataType:"json",data:{id:a}}).done(function(c){if(0==
c.Code){p.find("#"+f).remove();c=[];for(var d=0;d<t.length;d++)t[d].Id!=a&&c.push(t[d]);t=c}else z.Show(b["err.title"],c.Emsg)}).fail(function(){z.Show(b["err.title"],b["err net"])}).always(function(){e=1}))};A.find(".glyphicon-minus:first").click(function(b){h.toggle("fast");F.toggle("fast")});A.find(".glyphicon-wrench:first").click(function(a){r.Show(b["input section"],c,function(b,a){b.Hide();w(a)})});A.find(".glyphicon-remove:first").click(function(d){g.Show(b["msg warning"],b["chapter erase"]+
" - "+c+"<br>"+b["sure?"],function(b){b.Hide();y(a)})});l.find(".glyphicon-wrench:first").click(function(b){k.is(":visible")?(k.html(""),k.hide(),v.show()):(b=u.html(),n.ShowPreview(b))});l.find(".glyphicon-save:first").click(function(b){b=u.html();n.Save(b)})},x=function(a){t=[];$.ajax({url:"/Section/AjaxFind",type:"POST",dataType:"json",data:{panel:a}}).done(function(b){if(0==b.Code){p.html("");if(b=b.Data)for(var a=0;a<b.length;a++){var c=b[a];m(c.Id,c.Name,c.Str)}n.show();e=1}else p.html(b.Emsg)}).fail(function(){p.html(b["err net"])})};
x(a);d.find(".kBtnTop:first").click(function(b){location.href="#top"});d.find(".kBtnMenuHide:first").click(function(b){k.toggle("fast");q.toggle("fast")});d.find(".kBtnMenuSort:first").click(function(c){1==e&&0!=t.length&&h.Show(t,function(c){1==e&&(e=2,c=c.join("-"),$.ajax({url:"/Section/AjaxSort",type:"POST",dataType:"json",data:{sort:c}}).done(function(c){0==c.Code?(p.html(b["wait init data"]),x(a)):g.Show(b["err.title"],c.Emsg)}).fail(function(){g.Show(b["err.title"],b["err net"])}).always(function(){e=
1}))})});n.click(function(c){if(1==e){var d=b["new section"];e=2;$.ajax({url:"/Section/AjaxNew",type:"POST",dataType:"json",data:{panel:a,name:d}}).done(function(a){0==a.Code?m(a.Value,d):g.Show(b["err.title"],a.Emsg)}).fail(function(){g.Show(b["err.title"],b["err net"])}).always(function(){e=1})}});return{Rename:function(b){l.text(b)},Remove:function(){d.remove()}}},u=function(){var c=!0,h=$("#idBtnToggle"),e=$("#idBodyHide"),f=$("#idBodyView");h.click(function(b){e.toggle("fast");f.toggle("jqView")});
var d=$("#idListPanel"),l=[],k=function(){var b={};d.children("li").each(function(a,c){var d=$(c),e=d.attr("id");b[e]=d});for(var a=0;a<l.length;a++)d.append(b[l[a]])};d.sortable({update:function(a,e){if(!c)return!1;var m=d.sortable("toArray"),f=m.join("-");c=!1;$.ajax({url:"/Panel/AjaxSort",type:"POST",dataType:"json",data:{sort:f}}).done(function(a){0==a.Code?l=m:(k(),g.Show(b["err.title"],a.Emsg))}).fail(function(){k();g.Show(b["err.title"],b["err net"])}).always(function(){c=!0})}}).disableSelection();
var q=null,p=null,n=null,t=function(a,e){l.push(a);var f=$("<li class='list-group-item' id='"+a+"'><span data-id='"+a+"' class='glyphicon glyphicon-wrench kBtnSpan'></span><span data-id='"+a+"' class='glyphicon glyphicon-remove kBtnSpan'></span><a href='#panel-"+a+"'>"+e+"</a></li>"),m=f.find("a:first");d.append(f);q&&q(a,e);var h=function(d){d!=e&&c&&(c=!1,$.ajax({url:"/Panel/AjaxRename",type:"POST",dataType:"json",data:{id:a,name:d}}).done(function(c){0==c.Code?(e=d,m.text(e),p&&p(a,e)):g.Show(b["err.title"],
c.Emsg)}).fail(function(){g.Show(b["err.title"],b["err net"])}).always(function(){c=!0}))},k=function(a){c&&(c=!1,$.ajax({url:"/Panel/AjaxRemove",type:"POST",dataType:"json",data:{id:a}}).done(function(c){0==c.Code?(n&&n(a,e),f.remove()):z.Show(b["err.title"],c.Emsg)}).fail(function(){z.Show(b["err.title"],b["err net"])}).always(function(){c=!0}))};f.find(".glyphicon-wrench").click(function(a){r.Show(b["input chapter"],e,function(b,a){b.Hide();h(a)})});f.find(".glyphicon-remove").click(function(c){g.Show(b["msg warning"],
